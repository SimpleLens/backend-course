...
class HotelsRepository(BaseRepository):
    ...
    async def get_filtered_by_time(
        ...
        if title:
            query_to_get_hotels = query_to_get_hotels.filter(func.lower(HotelsModel.title).contains(title.strip().lower()))
        if location:
            query_to_get_hotels = query_to_get_hotels.filter(func.lower(HotelsModel.location).contains(location.strip().lower()))
