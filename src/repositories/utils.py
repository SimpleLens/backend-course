...
def rooms_ids_to_get(date_to: date, date_from: date, hotel_id: int | None = None):
    rooms_count = (
    ...
    query_for_hotels = select(RoomsModel.id).select_from(RoomsModel)
    ...
    rooms_to_get = select(rooms_left_table.c.room_id).filter(
