...
async def add_booking(Db: DbDep, user_id: UserIdDep, booking_data: BookingAddRequest):
    ...
    booking_data_to_add = BookingAdd(
        **booking_data.model_dump(), user_id=user_id, price=total_cost
    )

    added_booking = await Db.bookings.add_booking(
        booking_data_to_add, hotel_id=hotel_id
    )
    await Db.commit()
